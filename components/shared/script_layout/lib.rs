...
pub fn parse_resource_stylesheet_as_origin(
    shared_lock: &SharedRwLock,
    resource: Resource,
    origin: Origin,
) -> Result<ServoArc<Stylesheet>, &'static str> {
    let content = &resources::read_bytes(resource.clone());
    let url = Url::parse(&format!("chrome://resources/{:?}", resource.filename()))
        .ok()
        .unwrap();
...
pub fn parse_resource_document_stylesheet_as_origin(
    shared_lock: &SharedRwLock,
    resource: Resource,
    origin: Origin,
) -> Result<DocumentStyleSheet, &'static str> {
    parse_resource_stylesheet_as_origin(shared_lock, resource, origin).map(|stylesheet| DocumentStyleSheet(stylesheet))
}
