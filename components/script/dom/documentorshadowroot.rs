...
impl DocumentOrShadowRoot {
    ...
    pub(crate) fn add_stylesheet(
        ...
        debug_assert!(
            owner.is_none() || owner.unwrap().as_stylesheet_owner().is_some(),
            "Wat"
        );
    ...
    pub(crate) fn set_adopted_stylesheet(
        ...
        if common_prefix != adopted_stylesheets.len() {
            ...
            for sheet_to_remove in &adopted_stylesheets[common_prefix..] {
                ...
                if remove_set.insert(sheet_to_remove) {
                    owner.remove_stylesheet(sheet_to_remove.get_owner().as_deref(), sheet_to_remove.style_stylesheet_arc());
        ...
        for sheet in &incoming_stylesheets[common_prefix..] {
            if !stylesheet_set.insert(sheet) {
                ...
                // around.
                owner.remove_stylesheet(sheet.get_owner().as_deref(), sheet.style_stylesheet_arc());
            } else {
                sheet.add_adopter(owner.clone());
            }
            adopted_stylesheets.push(sheet.clone());

            owner.add_stylesheet(sheet.get_owner().as_deref(), sheet.style_stylesheet_arc().clone());
        }
