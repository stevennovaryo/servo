...
use script_layout_interface::parse_resource_stylesheet_as_origin;
...
impl HTMLDetailsElement {
    ...
    fn create_shadow_tree(&self, can_gc: CanGc) {
        ...
        root.upcast::<Node>()
            ...
            .unwrap();
        let details_ua_stylesheet = parse_resource_stylesheet_as_origin(
            link_element.upcast::<Node>().owner_doc().style_shared_lock(),
            Resource::DetailsCSS,
            Origin::Author,
        );
        link_element.set_stylesheet(details_ua_stylesheet.unwrap());
