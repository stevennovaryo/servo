...
use crate::geom::{PhysicalRect, PhysicalVec};
 ...
 pub(crate) struct ContainingBlockManagerRef<'a, T: ?Sized> {
     ...
     pub for_non_absolute_descendants: Ref<'a, T>,
     ...
     pub for_absolute_descendants: Option<Ref<'a, T>>,
     ...
     /// all descendants.
     pub for_absolute_and_fixed_descendants: Ref<'a, T>,
 }
 
 impl<'a, T> ContainingBlockManagerRef<'a, T> {
     pub(crate) fn get_containing_block_for_fragment(&self, fragment: &Fragment) -> Ref<'a, T> {
        let containing_block = if let Fragment::Box(box_fragment) = fragment {
         ...
         Ref::clone(containing_block)
     }
 
    pub(crate) fn new_for_non_absolute_descendants(
        &self,
        for_non_absolute_descendants: Ref<'a, T>,
    ) -> Self {
        ContainingBlockManagerRef {
            for_non_absolute_descendants,
            for_absolute_descendants: self.for_absolute_descendants.as_ref().map(|orig| Ref::clone(&orig)),
            for_absolute_and_fixed_descendants: Ref::clone(&self.for_absolute_and_fixed_descendants),
        }
    }
    ...
    pub(crate) fn new_for_absolute_descendants(
        &self,
        for_non_absolute_descendants: Ref<'a, T>,
        for_absolute_descendants: Ref<'a, T>,
    ) -> Self {
        ContainingBlockManagerRef {
            for_non_absolute_descendants,
    ...
    pub(crate) fn new_for_absolute_and_fixed_descendants(
        &self,
        for_non_absolute_descendants: Ref<'a, T>,
        for_absolute_and_fixed_descendants: Ref<'a, T>,
    ) -> Self {
        ContainingBlockManagerRef {
            for_non_absolute_descendants,
