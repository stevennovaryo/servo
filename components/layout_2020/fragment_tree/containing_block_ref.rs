...
pub(crate) struct ContainingBlockManagerRc<T: ?Sized> {
    ...
    pub for_non_absolute_descendants: Rc<T>,
    ...
    pub for_absolute_descendants: Option<Rc<T>>,
    ...
    /// all descendants.
    pub for_absolute_and_fixed_descendants: Rc<T>,
}

impl<T> ContainingBlockManagerRc<T> {
    pub(crate) fn get_containing_block_for_fragment(&self, fragment: &Fragment) -> Rc<T> {
    ...
    pub(crate) fn new_for_non_absolute_descendants(
        &self,
        for_non_absolute_descendants: Rc<T>,
    ) -> Self {
        ContainingBlockManagerRc {
            for_non_absolute_descendants,
            for_absolute_descendants: self.for_absolute_descendants.clone(),
            for_absolute_and_fixed_descendants: self.for_absolute_and_fixed_descendants.clone(),
        }
    }
    ...
    pub(crate) fn new_for_absolute_and_fixed_descendants(
        &self,
        for_non_absolute_descendants: Rc<T>,
        for_absolute_and_fixed_descendants: Rc<T>,
    ) -> Self {
        ContainingBlockManagerRc {
            for_non_absolute_descendants,
